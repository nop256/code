...
class TestListsCalorieList(asttest.ASTTest):

    def setUp(self):
        super().setUp("lists_calorie_list.py")
    ...
    def test_list(self):
        ...
        if len(list_elts) < 3:
            self.fail("You need at least 3 numbers in your list variable.")
        if any([not isinstance(elt, ast.Num) for elt in list_elts]):
            self.fail("All of the elements in the list must be numbers.")
