...
class bag:
    ...
    def insert(self, elt):
        ...
        self.root = insertR(elt, self.root)
        return True
    ...
    def retrieve(self, elt):
        ...
        return retrieveR(elt, self.root)

    def retrieveR(self, elt, current):
        if current is None: return False
        elif elt == current.elt: return current.elt
        elif elt < current.elt: return retrieveR(elt, current.l)
        elif elt > current.elt: return retrieveR(elt, current.r)
